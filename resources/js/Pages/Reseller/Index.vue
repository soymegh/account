...
import CreateModal from "@/Pages/Reseller/Create.vue";
...
function change_status(event) {
  if (event) {
      Inertia.post( route('api.reseller.change-status'), {
        "id" : event.target.id
      },{
        onSuccess: (page) => {
          Toast.fire({
...
function item_delete() {
    ...
    if ( select.length ) {
      ...
      Inertia.post( route('api.reseller.destroy') , {
        'data' : select
      },{
        onSuccess: (page) => {
          Toast.fire({
    ...
    <AppLayout>
      ...
      <div class="container mx-auto sm:px-6 lg:px-8 bg-white p-10 shadow-xl">
        ...
        <div class="bg-white overflow-hidden  sm:rounded-lg ">
            <Table :resource="resellers"
            :striped="true"
            :prevent-overlapping-requests="false"
            :input-debounce-ms="1000"
            :prevent-scroll="true" class="py-10">

                <template #cell(check)="{ item: resellers }">
                    <label 
                        class="text-teal-500 inline-flex justify-between items-center hover:bg-gray-200 px-2 py-2 rounded-lg cursor-pointer">
                        <input v-model="select" @click="add_check" :value = resellers.id type="checkbox" class="form-checkbox rowCheckbox focus:outline-none focus:shadow-outline">
                    </label>
                </template>.3

                <template #cell(status)="{ item: resellers }">
                  <label :for="resellers.id" class="inline-flex relative items-center cursor-pointer">

                    <input @click="change_status" v-if=" resellers.status == 'Active'" checked type="checkbox" value="" :id="resellers.id" class="sr-only peer z-50"/>

                    <input @click="change_status" v-else type="checkbox" value="" :id="resellers.id" class="sr-only peer"/>

                    <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                  </label>
                </template>

                <template #cell(action)="{ item: resellers }">
                    <div class="flex justify-center">
                      <div class="p-1 rounded bg-green-500 text-white hover:bg-green-600 ease transition cursor-pointer" @click="selected = resellers ; showModal = true" >
